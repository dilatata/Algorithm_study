...
'''
for문으로 이전 도시보다 주유 가격이 적으면 남은 길의 거리만큼 주유 구매
같으면 유지
가격이 더 많으면 이전가격* 다음 도시까지 거리

'''

n=int(input()) # 도시 수
roads = list(map(int, input().split())) # 각 도시 사이의 거리
costs = list(map(int, input().split())) # 각 도시 주유 가격

tot_cost = costs[0]*roads[0] # 첫 이동에 지불할 가격
c = costs[0] # 첫 도시의 주유 가격
distance = 0 # 도시 사이 이동 거리 적제

for i in range(1, n-1): # if 도시 수가 4- > 2번도시 3번도시가격과 비교하면 된다.
    # print('===',i,'===', costs[i], roads[i])
    if costs[i] <= c: # 전 도시보다 주유 가격이 적을 경우
        distance = roads[i] # 다음 도시까지 이동할 거리 
        tot_cost += distance*costs[i] # 이번에 주유할 가격 더하기
        c = costs[i] # 이번 도시의 주유 가격 = 지금까지 중 가장 적은 주유가격
        # print('---1----', tot_cost)
